...
export function playingCard(c,hidden=false) {
  ...
  return svg(`<rect x="1" y="1" width="118" height="166" rx="7" fill="${bg}" stroke="#ded8c8" stroke-width="2"/><rect x="4" y="4" width="112" height="160" rx="5" fill="none" stroke="#fff" stroke-opacity=".6"/><g fill="${color}" color="${color}" class="bp-pips">${middle.replace(/fill="#25282d"/g,`fill="${color}"`)}</g>${indices}${c.seal?`<circle cx="90" cy="120" r="12" fill="${{red:'#b73837',blue:'#3a79ab',gold:'#cb9a35',purple:'#9862a3'}[c.seal]}" stroke="#f3db9b" stroke-width="1.5"/>${text(90,124,'✦',13,'#f8e4bc')}`:''}${c.edition?'<path d="M5 52L115 14V39L5 77ZM5 129L115 91V102L5 140Z" fill="#fff" opacity=".18"/>':''}`)
}
