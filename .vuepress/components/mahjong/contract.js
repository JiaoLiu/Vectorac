...
export const RULE_VERSION = "scmj-1.0";
...
/**
 * Meld 副露
 * {kind:'peng', tile, from:seat, wild?}                         碰
 * {kind:'gang', gangType:'ming'|'an'|'bu', tile, from, wild?}   明杠/暗杠/补杠
 * from: 来源座位（暗杠为 null）
 * wild: 该副露里用了几张幺鸡赖子补位（0/undefined 表示纯真牌）。
 *   - 幺鸡局才有值；碰/杠允许「真牌 + 幺鸡」凑齐（如 1 张 1 万 + 1 只幺鸡碰 1 万）。
 *   - 碰带幺鸡、碰后补杠都不允许把幺鸡换回手里；明杠/暗杠带幺鸡时，
