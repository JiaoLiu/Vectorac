...
echo "  WS    $WS_PATH"
echo ""
echo "在 vectorac.com 的 server { }（443/ssl 块）里加一行 include 引用它："
echo "  include $NGINX_SNIPPET;"
echo "  # 自动插入 + 校验脚本见 README.md「部署到 vectorac.com / nginx」章节"
echo "  # 手动插入后： sudo nginx -t && sudo systemctl reload nginx"
echo ""
echo "注意：nginx 要允许 WebSocket 升级（配置里已含 Upgrade/Connection 头），"
echo "      且 proxy_read_timeout 要足够大，否则长连接会被 60s 掐断。"
echo "      改完确认 /api/rooms 返回 JSON（返回 HTML 说明还没生效）。"
echo ""
echo "================ 常用命令 ================"
